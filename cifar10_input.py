...
def distorted_inputs(data_dir, batch_size):
  ...
  Returns:
    ...
    labels: Labels. 1D tensor of [batch_size] size.
  """
  filenames = [os.path.join(data_dir, 'vtf_%d.bin' % i)
               for i in xrange(1, 3)]
  for f in filenames:
...
def inputs(eval_data, data_dir, batch_size):
  ...
  if not eval_data:
    ...
    filenames = [os.path.join(data_dir, 'vtf_1.bin')]
...
#                                                         height, width)
  ...
  # Subtract off the mean and divide by the variance of the pixels.
  float_image = reshaped_image#tf.image.per_image_standardization(reshaped_image)
